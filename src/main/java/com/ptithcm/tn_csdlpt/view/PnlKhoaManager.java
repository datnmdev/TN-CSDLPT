...
import com.ptithcm.tn_csdlpt.service.KhoaService;
 ...
 * @author MINHDAT
 */
public class PnlKhoaManager extends javax.swing.JPanel {

    private PnlManageBar pnlManageBar;
    private PnlKhoaInfo pnlKhoaInfo;

    private FrmMain frmMain;
    private String tabName;
    private String macs;

    //cái này dùng để làm có thể chỉnh sửa lại makh được
    private String makh;
     ...
     */
    public PnlKhoaManager(FrmMain frmMain, String tabName, String groupName) throws IOException, SQLException {
        initComponents();
        initMyComponents(tabName, groupName, frmMain);
        disableComponents(groupName);
        fillSideComboBox(groupName);
        addEvents();
        pnlKhoaInfo.getBtnKhoaManager().setEnabled(false);
    }

    public PnlKhoaManager() {
    }
...
//Thực thi các thao tác với các đối tượng được đưa vào undo, redo
    public void objectActionExecute(ObjectAction objectAction, Object[] row) {
        ...
        System.out.println(objectAction.getStatus().size() + " vô được chỗ thực thi");
        System.out.println(objectAction.getStatus().get(0));
    ...
    public void addEvents() {
        ...
        tblData.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                try {
                    ...
                    Object khoa = getObjectFromRow(selectedRow);
                    fillStudentField(khoa);
                    
                    //hàm này dùng để điều khiển sáng của nút tức là khi nhấn vào mã khoa có trong csdl thì nút sẽ sáng
                    try {
                        if (KhoaRepository.checkMaKhoa(makh)) {
                            pnlKhoaInfo.getBtnKhoaManager().setEnabled(true);
                        } else {
                            pnlKhoaInfo.getBtnKhoaManager().setEnabled(false);
                            
                        }
                    } catch (SQLException ex) {
                        Logger.getLogger(PnlKhoaManager.class.getName()).log(Level.SEVERE, null, ex);
                    }
        ...
        pnlManageBar.getBtnAdd().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                    ValidateFormService.validateFrmKhoaInfoAdd(pnlKhoaInfo, objectActions);

                    Khoa mh = new Khoa(pnlKhoaInfo.getTxtMaMH().getText(), pnlKhoaInfo.getTxtTenMH().getText(), macs);
                    int objectActionSize = objectActions.size();
                    DefaultTableModel tblDataModel = (DefaultTableModel) tblData.getModel();
                    ObjectAction objectAction = new ObjectAction(
                            ActionStatusEnum.INSERT, objectActionSize,
                            tblDataModel.getRowCount(), mh);
                    //                        Thêm đối tượng vào objectActions
                    objectActions.add(objectAction);
                    //                        Cập nhật lại bảng
                    Object[] row = {mh.getMaKH(), mh.getTenKH()};
        ...
        pnlManageBar.getBtnUpdate().addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                    ValidateFormService.validateFrmKhoaInfoUpdate(pnlKhoaInfo, makh);

                    Khoa monhoc = new Khoa(pnlKhoaInfo.getTxtMaMH().getText(), pnlKhoaInfo.getTxtTenMH().getText(), macs);
                    int objectActionIndex = -1;

                    for (int i = 0; i < objectActions.size(); ++i) {
                        Khoa mh = (Khoa) objectActions.get(i).getObjects().get(0);
                        if (makh.strip().equals(mh.getMaKH().strip())) {
                            objectActionIndex = i;
                            break;
                        }
                    }
                    ...
                    if (objectActionIndex != -1) {
                        ...
                        ObjectAction objectAction = new ObjectAction(
                                ...
                                (Khoa) objectActions.get(objectActionIndex).getObjects().get(0)
                        );
...
//                        Cập nhật dữ liệu cho bảng
                        DefaultTableModel tblDataModel = (DefaultTableModel) tblData.getModel();
                        Object[] row = {monhoc.getMaKH(), monhoc.getTenKH()};
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (objectActionIndex != -1) {
                    ...
                    ObjectAction objectAction = new ObjectAction(
                            ...
                            objectActionIndex, selectedRowIndex, monhoc
                    );
...
//                            Gọi lệnh save từ tầng service
                    KhoaService.saveAll(objectActions);
...
//                            Tiến hành load lại trang sau khi ghi thành công
                    ...
                    //  Load lại dữ liệu
                    KhoaController.renderData(PnlKhoaManager.this);
        ...
        pnlManageBar.getBtnUndo().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (!undoRedo.getUndoStack().isEmpty()) {
                    ...
                    Khoa khoa = (Khoa) objectActionUndo.getObjects().get(0);
                    Object[] row = {
                        khoa.getMaKH(), khoa.getTenKH()};
        ...
        pnlManageBar.getBtnRedo().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (!undoRedo.getRedoStack().isEmpty()) {
                    ...
                    Khoa monhoc = (Khoa) objectActionRedo.getObjects().get(0);
                    Object[] row = {
                        monhoc.getMaKH(), monhoc.getTenKH()};
...
//    Methods
    public void initMyComponents(String tabName, String groupName, FrmMain frmMain) throws IOException, SQLException {
        this.tabName = tabName;
        this.frmMain = frmMain;

        pnlManageBarContainer.add((pnlManageBar = new PnlManageBar()));
        pnlDataEntryFormContainer.add((pnlKhoaInfo = new PnlKhoaInfo()));

        pnlKhoaInfo.setGroupName(groupName);
    }
    ...
    public void fillTableForTeacher(List<ObjectAction> objectActions) {
        ...
        for (ObjectAction objectAction : objectActions) {
            ...
            row.add(((Khoa) objectAction.getObjects().get(0)).getMaKH());
            row.add(((Khoa) objectAction.getObjects().get(0)).getTenKH());
    ...
    public void fillStudentField(Object object) {
        ...
        //mỗi khi nhấn vào mã khoa sẽ vào đây để tránh trường họp không điền được mã khóa
        makh = kh.getMaKH();

        //sẽ chuyền mã khoa đã chọn mà biến mã khoa ở trong frmMain để chuyển bị taọ form quản lý lớp
        frmMain.setMakh(kh.getMaKH());

        pnlKhoaInfo.getTxtMaMH().setText(kh.getMaKH());
        pnlKhoaInfo.getTxtTenMH().setText(kh.getTenKH());
    }

    //hàm này dùng để gắn dữ liệu kiểu giáo viên vào kiểu dữ liệu object
    public Object getObjectFromRow(int rowIndex) throws ParseException {
        Object object;
    ...
    public PnlKhoaInfo getPnlKhoaInfo() {
        return pnlKhoaInfo;
    }

    public void setPnlKhoaInfo(PnlKhoaInfo pnlKhoaInfo) {
        this.pnlKhoaInfo = pnlKhoaInfo;
    }
